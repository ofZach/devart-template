...
//    }
    ...
    player.loadSound("a.wav");
    
    loadAudioToData("a.wav", samples);
...
void testApp::audioOut(float * buffer, int bufferSize, int nChannels){
    ...
    float * valArray = vals[ (int)(player.getPosition() * vals.size()) % vals.size()].vals;
...
//    }
    ...
    for (int j = 0; j < 12; j++){
        sinAngleAdder[j] =  freqs[j] * TWO_PI / 44100.0;
    }
    ...
    for (int i = 0; i < bufferSize; i++){
        ...
        for (int j = 0; j < 12; j++){
            ...
            float volume = powf(valArray[j] / maxVal, 2.0);
...
void testApp::draw(){
    ...
    for (int i = 0; i < 12; i++){
        ...
        for (int j = 0; j < 200; j++){
            ...
            float val = vals[ (int)(pctMe * vals.size()) % vals.size()].vals[i] / maxVal;
